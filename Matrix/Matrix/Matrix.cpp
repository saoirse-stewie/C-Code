...
#endif
	//	m = mx.GetM();//get the amount of row elements and storing in the dynamic array
	///	n = mx.GetN();//get the amount of column elements and storing in the dynamic array
	mat = new double *[m];//dynamically creates memory on the heap for row
	if (mat != nullptr)
	{
...
const Matrix Matrix:: operator +(const Matrix& ma)const
{
	...
	{
		std::cout << "Illegal input; Matrix must be equal Length" << std::endl;
	}
	matrix_cnt++;
}
const Matrix Matrix:: operator -(const Matrix& ma)const
...
const Matrix Matrix :: operator ==(const Matrix &ma)const
{
	...
	if (mat != nullptr)
	{
		for (int i = 0; i < ma.GetM(); i++)
		{
			...
			for (int j = 0; j < ma.GetN(); j++)
			{
				mat[i][j] = ma.GetElement(i, j);//set the elements from two arg constructor and put them into copy array 
				equal = true;
			}
		}
	}
	...
	if (this != &ma)// check that my address is not the same as the matrix coming in
	{
		...
		if (mat != nullptr)
		{
			for (int i = 0; i < ma.GetM(); i++)
			{
				...
				for (int j = 0; j < ma.GetN(); j++)
				{
					//result.SetElement(i,j, ma.GetElement(i, j));//set the elements from two arg constructor and put them into copy array 
